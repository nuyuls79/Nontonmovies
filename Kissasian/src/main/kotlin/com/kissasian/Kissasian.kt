...
class Kissasian : MainAPI() {
    ...
    override val hasMainPage = false // <--- mainPage OFF
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val status = getStatus(document.selectFirst("div.info-content div.spe span")?.ownText()?.replace(":", "")?.trim() ?: "")
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        document.selectFirst("div.player-embed iframe")?.attr("src")?.takeIf { it.isNotBlank() }?.let { iframe ->
        ...
        for (opt in mirrorOptions) {
            ...
            try {
                ...
                mirrorDoc.selectFirst("div.player-embed iframe")?.attr("src")?.takeIf { it.isNotBlank() }?.let { iframe ->
